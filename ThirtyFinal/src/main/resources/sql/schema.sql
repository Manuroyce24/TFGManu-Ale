...
(ID BIGINT NOT NULL, -- Identificador único de la solicitud de amistad
SENDER_ID BIGINT NOT NULL, -- Identificador del usuario que envía la solicitud
RECEIVER_ID BIGINT NOT NULL, -- Identificador del usuario que recibe la solicitud
STATUS VARCHAR(255) NOT NULL, -- Estado de la solicitud (PENDING, ACCEPTED, REJECTED)
CREATION_DATE TIMESTAMP NOT NULL, -- Fecha y hora de creación de la solicitud
PRIMARY KEY (ID),
...
REFERENCES USERS, -- La clave foránea referencia a la tabla USERS
CONSTRAINT UK_REQUEST UNIQUE (SENDER_ID, RECEIVER_ID) ); -- Cada combinación de SENDER_ID y RECEIVER_ID debe ser única
